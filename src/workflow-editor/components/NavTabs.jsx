...
export const NavTabs = memo((props) => {
  const { defaultValue, value, options, onChange } = props
  const [inputValue, setInputValue] = useState(defaultValue || value || props.options?.[0]?.key)
        ...
        options?.map((option, index) => {
          return (
            ...
            >
              <NavIcon
                role="img"
                className={classNames("anticon", option.key === inputValue ? "selected" : undefined)}
              >
                {index + 1}
              </NavIcon>
              {option.label}
